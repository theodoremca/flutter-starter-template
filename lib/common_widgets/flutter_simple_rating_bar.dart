...
class RatingBar extends StatefulWidget {
  ...
  RatingBar(
      {this.starCount = 5,
      this.rating = 0.0,
      this.color = const Color(0xffE5A800),
      this.icon,
      this.size = 24,
...
class _RatingBarState extends State<RatingBar> {
  ValueNotifier<double> _ratingNotifier = ValueNotifier<double>(1);

  ValueNotifier<bool> _isIndicatorNotifier = ValueNotifier<bool>(false);
  double _rating = 1;
  ...
  Widget build(BuildContext context) {
    return new Material(
        ...
        child: ValueListenableBuilder<bool>(
            builder: (context, isIndicator, child) {
